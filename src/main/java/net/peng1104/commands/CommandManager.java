...
import net.peng1104.events.AsyncCommandEvent;
import net.peng1104.events.CodeCommandEvent;
import net.peng1104.profiles.Profile;
import net.peng1104.utils.MinecraftVersion;
...
public class CommandManager implements Listener {
    ...
    static {
        try {
            ...
            InsperCodePlugin.getInstance().getLogger()
    ...
    private void onPlayerChat(@NotNull AsyncPlayerChatEvent event) {
        ...
        Profile sender = InsperCodePlugin.getInstance().getProfileManager()
    ...
    private void onConsoleCommand(@NotNull ServerCommandEvent event) {
        ...
        if (executeAsyncCommand(InsperCodePlugin.getInstance().getProfileManager().getConsole(),
                label, args, async)) {
        ...
        event.setCancelled(
                executeSyncCommand(InsperCodePlugin.getInstance().getProfileManager().getConsole(),
                        label, args, async));
    }
    ...
    private boolean executeAsyncCommand(@NotNull Profile sender, @NotNull String label,
        ...
        if (inAsyncThread) {
            ...
            }.runTaskAsynchronously(InsperCodePlugin.getInstance());
        }
        return true;
    }
    ...
    private boolean executeSyncCommand(@NotNull Profile sender, @NotNull String label,
        ...
        if (inAsyncThread) {
            ...
            }.runTask(InsperCodePlugin.getInstance());
        }
        else {
            event.getCommand().preExecute(event.getSender(), label, args);
        }
